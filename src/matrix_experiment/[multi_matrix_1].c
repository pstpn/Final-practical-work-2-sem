// Умножение квадратных матриц с их транспонированием
...
void transpose(double matrix[][MAX_SIZE], size_t n)
{
    double trash;
...
double (*s_matrix)[MAX_SIZE], double (*new_matrix)[MAX_SIZE], size_t n)
{
    ...
    for (size_t num = 0; num < n; num++)
    {
        ...
        for (size_t i = 0; i < n; i++)
        {
            for (size_t j = 0; j < n; j++)
            {
                summ += (double)f_matrix[j][i] * s_matrix[num][j];
            }
